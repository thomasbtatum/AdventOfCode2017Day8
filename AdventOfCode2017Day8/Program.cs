 ...
 */
namespace AdventOfCode2017Day8
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string input = @"hwv inc 149 if clj >= -5
...
l dec 367 if uix >= -3376";
        ...
        private static void Part1(string[] lines)
        {
            ...
            foreach (var line in lines)
            {
                var cmds = line.Split(' ');
                var Id = cmds[4];
                var checkIdCmd = cmds[5];
                var staticVal = Convert.ToInt32(cmds[6]);
                ...
                {
                    lookup.Add(Id, checkVal);
                }

                //<, >, >=, ==, <=, !=
                if (
                    ((checkIdCmd == "<") && (checkVal < staticVal)) ||
                    ((checkIdCmd == ">") && (checkVal > staticVal)) ||
                    ((checkIdCmd == ">=") && (checkVal >= staticVal)) ||
                    ((checkIdCmd == "==") && (checkVal == staticVal)) ||
                    ((checkIdCmd == "<=") && (checkVal <= staticVal)) ||
                    ...
                    var register = cmds[0];
                    var incdeccmd = cmds[1];
                    var incdecval = Convert.ToInt32(cmds[2]);

                    var lookupVal = 0;
                    if (lookup.ContainsKey(register))
                    {
                        lookupVal = lookup[register];
                    } else
                    {
                        lookup[register] = lookupVal;
                    }
                    int maxCheck = 0;
                    if(incdeccmd == "inc")
                    {
                        maxCheck = lookupVal + incdecval;
                        lookup[register] = maxCheck;
                    } else
                    {
                        maxCheck = lookupVal - incdecval;
                        lookup[register] = maxCheck;
